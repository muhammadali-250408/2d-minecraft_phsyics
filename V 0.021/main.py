...
class mainGame():
    ...
    def update(self):
        ...
        for collider in self.world_colliders:
            if self.player.collider.colliderect(collider) and self.player.y < collider.y:
                self.player.isOnFloor = True
                self.player.isJumping = False
                self.player.y_vel = 0
                print("HITTING FLOORHITTING FLOORHITTING FLOORHITTING FLOOR")
                break

        # SIDE WALL COLLISION HANDLING
        # HANDLING LEFT SIDE COLLISIONS
        for collider in self.world_colliders:
            if self.player.left_collider.colliderect(collider):
                self.player.x += 2
                break
            # HANDLING RIGHT SIDE COLLISIONS
            elif self.player.right_collider.colliderect(collider):
                self.player.x -= 2
                break
